...
- Une exécution de `pytest` locale montre 2 tests verts et 3 ignorés, confirmant le bon fonctionnement des chemins de sélection du lanceur tout en signalant l’absence de certaines dépendances optionnelles.
...
- **Couverture minimale** : les tests couvrent les chemins de sélection du lanceur et permettent de détecter l’indisponibilité de dépendances optionnelles sans faire échouer la suite.
...
2. Intégrer un test d’intégration léger qui lance Uvicorn sur un port éphémère avec `BACKEND_SKIP_INSTALL=1` et vérifie une réponse basique (healthcheck) pour s’assurer que le code démarre réellement.
3. Documenter un mécanisme de retry ou de configuration de miroirs PyPI pour les environnements réseau restreints.

## Note globale
Sur la solution actuelle après modifications et exécution des tests : **7/10**. Le bootstrap est plus résilient et observé, mais il manque encore des garde-fous de configuration et des tests d’intégration couvrant le démarrage réel de l’application.
